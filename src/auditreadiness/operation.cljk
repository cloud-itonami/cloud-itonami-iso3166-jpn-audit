(ns auditreadiness.operation
  "OperationActor -- one JPN-AUDIT audit-readiness compliance operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (AuditReadiness-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Audit-Readiness
  Compliance Governor (:govern) and the rollout phase gate (:decide)
  ...
  governor is clean -- see `auditreadiness.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [auditreadiness.auditreadinessllm :as auditreadinessllm]
            [auditreadiness.governor :as governor]
            [auditreadiness.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (auditreadinessllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (auditreadinessllm/-advise advisor store request)]
            {:proposal p :audit [(auditreadinessllm/trace request p)]})))
