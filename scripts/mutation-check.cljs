...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "audit-mutation-" suffix ".edn")))
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-jbaudit-go-jp" "host.www-japaneselawtranslation-go-jp"
            "law.audit-act" "jbaudit.report" "jlt.audit-act-en" "jbaudit.law-page" "jlt.home"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.audit-act" (fn [e] (dissoc e :egov/law-id)))
    ...
    :mutate (fn [data]
              (mapv (fn [e]
                      (if (and (= :page-identity (:source/verify e))
                               (not= "jbaudit.report" (:source/id e)))
  ...
  [{:id "nonexistent-law" :on :laws-only
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.audit-act"
                                  (fn [e] (assoc e :egov/law-title "会計検査院条例")))
                    (into UNROUTABLE-HOST))))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "page-title-drift"
    :why "a real page recorded under a title it does not carry"
    :mutate #(alter-entity % "jbaudit.report" (fn [e] (assoc e :page/title "会計検査院")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "jbaudit.law-page"
                           (fn [e] (assoc e :page/must-contain ["会計検査院組織法"])))
