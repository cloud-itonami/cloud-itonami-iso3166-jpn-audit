...
;;
;; THE HOST MEASUREMENTS THAT SHAPE THIS REGISTER (re-measured every run):
;;
;; www.jbaudit.go.jp answers **404** -- not 403, not a redirect -- for a path
;; that does not exist, and the 404 page is a SOFT 404: a full site-chrome
;; page whose <title> is the SITE title, identical to the front page's. On
;; that host the title separates nothing; only the status does. It also does
;; NOT redirect http:// to https://. www.japaneselawtranslation.go.jp answers
;; 404 with a short "Error - Japanese Law Translation" page whose title DOES
;; separate it from real pages, and DOES redirect http to https. Neither host
;; has been observed to serve a bot challenge; the challenge detector below
;; is kept anyway, because a detector whose register has never measured a
;; challenge is exactly what you want already in place when one appears.
;;
...
;;                          (measured on the JLT host, which redirects http to
;;                          https; the Board of Audit host does NOT redirect,
;;                          so the self-test walks the pool and requires SOME
;;                          probe to demonstrate the branch)
...
(defn- self-test-redirect [probes]
  ;; Ask for the http:// form of pages the register cites over https. A host
  ;; that redirects lands on the https URL, which is not the URL asked for and
  ;; nothing declares a redirect -- the branch that catches a 2xx arriving from
  ;; somewhere else.
  ;;
  ;; MEASURED, NOT ASSUMED: the two hosts in this register behave differently.
  ;; www.japaneselawtranslation.go.jp redirects http to https; www.jbaudit.go.jp
  ;; does NOT -- its http form answers 404 at the http URL (see facts.edn). The
  ;; METI register could assume "all cited agency hosts redirect"; this one
  ;; cannot, so the self-test walks the pool and requires SOME probe to come
  ;; back with the redirect reason. If none does, the branch is unexercised and
  ;; the run refuses -- a self-test that passes by not running is the failure
  ;; this file is built against.
  (if (empty? probes)
    (p/resolved {:ok false :fatal true
                 :why "the self-test pool is empty, so the redirect branch can never be exercised"})
    (p/loop [rem (vec probes) results []]
      (if (empty? rem)
        (let [bad (str/join "; " results)]
          (p/resolved {:ok false
                       :why (str "no probe in the pool demonstrated the redirect branch: " bad
                                 ". If a host's http behaviour changed, re-measure facts.edn.")}))
        (let [probe (first rem)
              https-url (:source/url probe)
              http-url (str/replace https-url #"^https://" "http://")]
          (p/let [r (check-page (assoc probe :source/url http-url) false)]
            (if (= :unexpected-redirect (:reason r))
              (p/resolved {:ok true
                           :detail (str "http:// form of " https-url " (lands on the https URL) -> unexpected-redirect")})
              (p/recur (vec (rest rem))
                       (conj results (str (:source/id probe) " came back " (pr-str (:verdict r))
                                          " / " (pr-str (:reason r))))))))))))
          ...
          (p/let [selfs (serially (fn [f] (f))
                                  (cond-> [self-test-law self-test-challenge]
                                    ...
                                    (into [#(self-test-title (pick 0))
                                           #(self-test-redirect (remove nil? (map pick [0 1 2 3])))
